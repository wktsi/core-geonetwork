...
public class MetadataSpecsTest extends AbstractSpringDataTest {
    ...
    public void testHasMetadataType() throws Exception {
        ...
        assertEquals(md1.getId(), _repository.findOne(hasType(MetadataType.METADATA)).get().getId());
        assertEquals(md2.getId(), _repository.findOne(hasType(MetadataType.SUB_TEMPLATE)).get().getId());
        assertEquals(md3.getId(), _repository.findOne(hasType(MetadataType.TEMPLATE)).get().getId());
    }
    ...
    public void testHasSchemaId() throws Exception {
        ...
        assertEquals(md1.getId(), _repository.findOne(hasSchemaId(schemaId1)).get().getId());
        assertEquals(md2.getId(), _repository.findOne(hasSchemaId(schemaId2)).get().getId());
    }
    ...
    private void assertFindsCorrectMd(Metadata md1, Specification<Metadata> spec, boolean addNewMetadata) {
        ...
        assertEquals(1, found.size());
        assertEquals(md1.getId(), found.get(0));
    }
}
