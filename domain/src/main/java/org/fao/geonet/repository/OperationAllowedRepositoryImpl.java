 ...
 * @author Jesse
 */
public class OperationAllowedRepositoryImpl extends GeonetRepositoryImpl<OperationAllowed, OperationAllowedId>
	...
	public List<OperationAllowed> findAllWithOwner(int userId,
		...
		Predicate userEqualsPredicate = cb.equal(metadataRoot.get(Metadata_.sourceInfo).get(MetadataSourceInfo_.owner),
				userId);
		Predicate mdIdEquals = cb.equal(metadataRoot.get(Metadata_.id),
				operationAllowedRoot.get(OperationAllowed_.id).get(OperationAllowedId_.metadataId));
	...
	public int deleteAllByIdAttribute(SingularAttribute<OperationAllowedId, Integer> idAttribute, int id) {
		...
		final Query query = _entityManager
				.createQuery("DELETE FROM " + opAllowedEntityName + " where " + idPath + " = " + id);
