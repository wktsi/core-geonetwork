 ...
 */
public class MetadataCategoryRepositoryImpl extends GeonetRepositoryImpl<MetadataCategory, Integer>
		implements MetadataCategoryRepositoryCustom {
	...
	public void deleteCategoryAndMetadataReferences(int id) {
		 ...
		 * Start of HACK.
		 *
		 * The following select seems to be needed so that the delete from below will
		 * actually delete elements... At least in the unit tests.
		 */
		final Query nativeQuery2 = _entityManager
				.createNativeQuery("Select * from " + Metadata.METADATA_CATEG_JOIN_TABLE_NAME + " WHERE "
		...
		final Query nativeQuery = _entityManager
				.createNativeQuery("DELETE FROM " + Metadata.METADATA_CATEG_JOIN_TABLE_NAME + " WHERE "
