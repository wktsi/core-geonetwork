...
/**
 * Override delete methods in
 * {@link org.springframework.data.jpa.repository.support.SimpleJpaRepository}
 * so that full subtree is deleted and implement
 * {@link HarvesterSettingRepositoryCustom}.
 * <p/>
 * This class is not a typical *Impl because it needs to override the delete
 * methods in
 * {@link org.springframework.data.jpa.repository.support.SimpleJpaRepository}.
 * In order to do this you have to create a subclass of
 * {@link org.springframework.data.jpa.repository.support.SimpleJpaRepository}
 * (or {@link GeonetRepositoryImpl} which is subclass) and have
 * {@link GeonetRepositoryFactoryBean} return the custom implementation.
 * <p/>
 * An alternative would be to use aspectJ and the around cut-point to modify the
 * behaviour of the method. I decided on this way because it is more common and
 * better understood techniques.
 * <p/>
 * <p/>
 * In addition to overriding the delete methods this class also implements the
 * {@link HarvesterSettingRepositoryCustom} interface. These methods are not in
 * a normal *Impl class because the extra class is not needed and the delete
 * methods call methods in that interface.
 * <p/>
 * User: Jesse Date: 10/25/13 Time: 7:53 AM
 */
public class HarvesterSettingRepositoryImpl extends GeonetRepositoryImpl<HarvesterSetting, Integer>
	...
	/**
	 * Overrides the implementation in
	 * {@link org.springframework.data.jpa.repository.support.SimpleJpaRepository}.
