...
/**
 * Implementation for all {@link Metadata} queries that cannot be automatically
 * generated by Spring-data.
 *
 * @author Jesse
 */
public class MetadataRepositoryImpl extends GeonetRepositoryImpl<Metadata, Integer>
		implements MetadataRepositoryCustom<Metadata> {
	...
	public Metadata findOne(String id) {
		try {
			...
			throw new IllegalArgumentException(
					"id parameter of findByIdString must be parsable to an integer.  It was '" + id + "'");
		}
	}
	...
	public List<SimpleMetadata> findAllSimple(String id) {
		Query query = _entityManager.createQuery("select new org.fao.geonet.repository.SimpleMetadata("
	...
	public Element findAllUuidsAndChangeDatesAndSchemaId(List<Integer> ids, @Nonnull Pageable pageable) {
		...
		cbQuery.multiselect(root.get(Metadata_.uuid), root.get(Metadata_.dataInfo).get(MetadataDataInfo_.changeDate),
				root.get(Metadata_.dataInfo).get(MetadataDataInfo_.schemaId));

		cbQuery.where(root.get(Metadata_.id).in(ids),
				cb.equal(root.get(Metadata_.dataInfo).get(MetadataDataInfo_.type_JPAWorkaround), 'n'));
