...
/**
 * Implementation for all {@link User} queries that cannot be automatically
 * generated by Spring-data.
 *
 * @author Jesse
 */
public class UserRepositoryImpl extends GeonetRepositoryImpl<User, Integer> implements UserRepositoryCustom {
	...
	public User findOneByEmail(final String email) {
		...
		if (resultList.size() > 1) {
			Log.error(Constants.DOMAIN_LOG_MODULE,
					"The database is inconsistent.  There are multiple users with the email address: " + email);
		}
	...
	public User findOneByEmailAndSecurityAuthTypeIsNullOrEmpty(final String email) {
		...
		query.where(cb.and(cb.isMember(email, root.get(User_.emailAddresses)),
	...
	public User findOneByUsernameAndSecurityAuthTypeIsNullOrEmpty(final String username) {
		...
		query.where(cb.and(cb.equal(usernamePath, username),
				cb.or(cb.isNull(authTypePath), cb.equal(cb.trim(authTypePath), ""))));
		List<User> results = _entityManager.createQuery(query).getResultList();
	...
	public List<Pair<Integer, User>> findAllByGroupOwnerNameAndProfile(@Nonnull final Collection<Integer> metadataIds,
		...
		Predicate userToGroupPredicate = cb.equal(userGroupRoot.get(UserGroup_.id).get(UserGroupId_.userId),
				userRoot.get(User_.id));

		Predicate basePredicate = cb.and(metadataPredicate, ownerPredicate, userToGroupPredicate);
		if (profile != null) {
			Expression<Boolean> profilePredicate = cb.equal(userGroupRoot.get(UserGroup_.id).get(UserGroupId_.profile),
					profile);
